...
@FeignClient(name = "${client.store-service.name}", url = "${client.store-service.url}", configuration = SystemFeignConfig.class)
public interface StoreSystemFeignClient {

    @PostMapping("/internal/v1/stores/regions")
    ApiResponse<List<StoreRegionInfoResponse>> fetchStoresRegionByIds(@RequestBody List<Long> storeIds);

    @PostMapping("/internal/v1/stores/search")
